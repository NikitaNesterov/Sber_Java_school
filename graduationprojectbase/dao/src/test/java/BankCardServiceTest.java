...
public class BankCardServiceTest {
    ...
    @Test
    public void shouldGetPrivateCustomerTest() {
        int customer = bankCardService.createCustomer(privateCustomer);
        bankCardService.saveBankProduct(customer, bankCard);
        assertEquals(privateCustomer, bankCardService.getPrivateCustomer(bankCard));
    }

    @Test
    public void shouldGetPrivateCustomerTest1() {
        int customer = bankCardService.createCustomer(privateCustomer);
        bankCardService.saveBankProduct(customer, bankCard);
        PrivateCustomer privateCustomer1 = new PrivateCustomer("Раиса", "29.07.1982", "mirnes1809@hotmail.com");
        assertNotSame(privateCustomer1, bankCardService.getPrivateCustomer(bankCard));
    }

    @Test
    public void shouldGetPrivateCustomerTest2() {
        int customer = bankCardService.createCustomer(privateCustomer);
        bankCardService.saveBankProduct(customer, bankCard);
        BankCard bankCard1 = new BankCard(mir, 2015, 10, 11);
        assertNull(bankCardService.getPrivateCustomer(bankCard1));
    }
    ...
    public void shouldDeleteCard() {
        int customer = bankCardService.createCustomer(privateCustomer);
        bankCardService.saveBankProduct(customer, bankCard);
        bankCardService.deleteCard(bankCard.getCardNumber());
        assertEquals(false, bankHashMap1.containsKey(bankCard.getCardNumber()));
    }
